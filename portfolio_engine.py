...
from core.metrics import (
    ...
    PortfolioStats,
    RiskMetrics,
    annual_returns as build_annual_returns,
    ...
    cumulative_returns as build_cumulative_returns,
...
def run_portfolio_analysis(
    ...
    if selected_strategy not in strategy_results:
        raise PortfolioError(f"Selected strategy '{selected_strategy}' was not produced by the optimizer.")

    selected_result = strategy_results[selected_strategy]
    aligned_returns = align_strategy_and_benchmark_returns(
        selected_result.portfolio_returns.rename("Portfolio"),
    ...
    return PortfolioAnalysisResult(
        ...
        cumulative_returns=build_cumulative_returns(aligned_returns),
        annual_returns=build_annual_returns(aligned_returns),
        drawdown_periods=worst_drawdown_periods(selected_result.portfolio_returns),
        summary_table=comparison_summary_frame(strategy_results),
        strategy_returns=strategy_returns,
        strategy_cumulative_returns=build_cumulative_returns(strategy_returns),
        strategy_annual_returns=build_annual_returns(strategy_returns),
...
def get_data(tickers: list[str], start: str, end: str) -> pd.DataFrame:
    """Fetch clean adjusted-close prices for the optimizer."""
    return fetch_prices(tickers, start, end, min_rows=60)


def get_benchmark_returns(benchmark: str, start: str, end: str) -> pd.Series:
    """Fetch benchmark returns as daily simple returns."""
    return _get_benchmark_returns(benchmark, start, end).rename("Benchmark")
...
def optimize_strategy(
    ...
    """Optimize one strategy and convert it to the dashboard result contract."""
...
def compare_strategies(
    ...
    """Optimize each built-in strategy against the same cleaned return history."""
    previous_weights = previous_weights or {}
    results = {}
...
def build_strategy_result(
    ...
    """Convert optimizer output into dashboard metrics, returns, and diagnostics."""
    clean = clean_returns(returns)
    _, covariance = estimate_annualized_inputs(clean)
    portfolio_returns = compute_portfolio_returns(clean, optimizer_result.weights, name=strategy)
    return StrategyResult(
        ...
        annual_returns=build_annual_returns(portfolio_returns),
...
def strategy_returns_frame(results: Mapping[str, StrategyResult]) -> pd.DataFrame:
    """Return aligned realized returns by strategy."""
...
def strategy_risk_contribution_frame(results: Mapping[str, StrategyResult]) -> pd.DataFrame:
    """Return variance risk contribution as a ticker-by-strategy table."""
...
def optimize_portfolio(
    ...
    """Backward-compatible helper returning weights and stats for Max Sharpe."""
...
def get_portfolio_correlation_matrix(result: PortfolioAnalysisResult) -> pd.DataFrame:
    """Return the correlation matrix for all selected optimizer tickers."""
...
def monte_carlo_simulation(
    ...
    """Simulate future portfolio paths from historical mean/covariance estimates."""
    if simulations < 100:
    ...
    clean = clean_returns(returns)
    aligned_weights = weights.reindex(clean.columns).fillna(0.0).to_numpy()
    mean_returns = clean.mean().to_numpy()
...
def monte_carlo_display_values(
    ...
    """Return display-ready Monte Carlo values for the Streamlit UI."""
